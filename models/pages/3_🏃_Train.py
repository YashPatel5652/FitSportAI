...
if app_mode == "About":
    ...
    if st.session_state['type']=='Start':
        ...
        fig = go.Figure(data=[go.Bar(x=['Left Dumbbell'], y=[calories1], name='Calories Burned')])

        fig.add_trace(go.Bar(x=['Left Dumbbell'], y=[goal_calorie1], name='Goal Calorie'))

        # Set chart layout
        fig.update_layout(
            title='Calories Burned for Left Dumbbell',
        ...
        workout_data = {
        "exercise": "Left Dumbbell",
        "reps": st.session_state.counter1,
        "calories": calories1,
        "goal_calories": goal_calorie1,
    ...
    class angleFinder:
        ...
        def angle(self):
            if len(self.lmlist) != 0:
                ...
                # calculating angle for right arm
                leftHandAngle = math.degrees(math.atan2(y3 - y2, x3 - x2) -
                                            math.atan2(y1 - y2, x1 - x2))
    ...
    if st.session_state['type']=='Start2':
        ...
        fig = go.Figure(data=[go.Bar(x=['Right Dumbbell'], y=[calories2], name='Calories Burned')])

        fig.add_trace(go.Bar(x=['Right Dumbbell'], y=[goal_calorie2], name='Goal Calorie'))

        # Set chart layout
        fig.update_layout(
            title='Calories Burned for Right Dumbbell',
            xaxis_title='Exercise',
            yaxis_title='Calories Burned'
        )
        ...
        workout_data = {
            "exercise": "Right Dumbbell",
            "reps": st.session_state.counter2,
            "calories": calories2,
            "goal_calories": goal_calorie2,
            "timestamp": SERVER_TIMESTAMP
            }
        user_id = st.session_state["user_id"]
        db.collection("users").document(user_id).collection("workouts").add(workout_data)
    ...
    class angleFinder:
        ...
        def angle(self):
            if len(self.lmlist) != 0:
                ...
                rightHandAngle = int(np.interp(rightHandAngle, [42, 143], [100, 0]))
    ...
    if st.session_state['type']=='Start3':
        ...
        while cap.isOpened():
            ...
            cv2.rectangle(img, (8, int(leftval)), (50, 400), (255, 0, 0), -1)


            # Drawing left rectangle and putting text
            cv2.putText(img, 'L', (900,195),cv2.FONT_HERSHEY_DUPLEX, 1, (255, 0, 0),5)
            cv2.rectangle(img, (882, 200), (932, 400), (0, 255, 0), 5)
            cv2.rectangle(img, (882, int(rightval)), (932, 400), (255, 0, 0), -1)

            if value_left > 70:
                cv2.rectangle(img, (882, int(rightval)), (932, 400), (0, 0, 255), -1)
        ...
        calories3=2.0*weight3/st.session_state.counter3
        ...
        fig = go.Figure(data=[go.Bar(x=['Sqauts'], y=[calories3], name='Calories Burned')])

        fig.add_trace(go.Bar(x=['Sqauts'], y=[goal_calorie3], name='Goal Calorie'))

        # Set chart layout
        fig.update_layout(
            title='Calories Burned For Sqauts',
            xaxis_title='Exercise',
            yaxis_title='Calories Burned'
        )
        ...
        workout_data = {
            "exercise": "Sqauts",
            "reps": st.session_state.counter3,
            "calories": calories3,
            "goal_calories": goal_calorie3,
            "timestamp": SERVER_TIMESTAMP
            }
        user_id = st.session_state["user_id"]
        db.collection("users").document(user_id).collection("workouts").add(workout_data)
    ...
    if st.session_state['type']=='Start4':
        ...
        fig = go.Figure(data=[go.Bar(x=['Pushups'], y=[calories4], name='Calories Burned')])

        fig.add_trace(go.Bar(x=['Pushups'], y=[goal_calorie4], name='Goal Calorie'))

        # Set chart layout
        fig.update_layout(
            title='Calories Burned for Pushups',
            xaxis_title='Exercise',
            yaxis_title='Calories Burned'
        )
        ...
        workout_data = {
            "exercise": "Push Up",
            "reps": st.session_state.counter4,
            "calories": calories4,
            "goal_calories": goal_calorie4,
            "timestamp": SERVER_TIMESTAMP
            }
        user_id = st.session_state["user_id"]
        db.collection("users").document(user_id).collection("workouts").add(workout_data)
    ...
    if st.session_state['type']=='Start5':
        ...
        workout_data = {
            ...
            "reps": st.session_state.counter5,
            "calories": calories5,
            "goal_calories": goal_calorie5,
            "timestamp": SERVER_TIMESTAMP
            }
        user_id = st.session_state["user_id"]
        db.collection("users").document(user_id).collection("workouts").add(workout_data)
